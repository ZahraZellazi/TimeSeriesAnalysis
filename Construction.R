...
data <- read_excel("/Users/zahra/Desktop/4DS/sem2/Series Temporelles /Projet/construction spending S C3.xlsx", skip = 6)
...
data_clean$Value <- zoo::na.approx(data_clean$Value, na.rm = FALSE)

# Supprimer les NA restants si présents
data_clean <- data_clean %>% drop_na(Value)

# Étape 6 : Créer une tsibble
data_tsibble <- data_clean %>%
...
construction_ts <- ts(data_clean$Value, start = c(2002, 1), frequency = 12)
...
# Étape 11 : Décomposition classique
decomp_additive <- decompose(construction_ts, type = "additive")
...
# Lissage par modèle linéaire
temps <- time(construction_ts)
modele_lm <- lm(construction_ts ~ temps)
summary(modele_lm)
plot(construction_ts, main = "Ajustement linéaire", col = "gray")
abline(modele_lm, col = "red", lwd = 2)

# Lissage par modèle polynomial (degré 2)
modele_poly <- lm(construction_ts ~ poly(temps, 2, raw = TRUE))
summary(modele_poly)
df <- data.frame(temps = as.numeric(temps),
                 construction = as.numeric(construction_ts),
                 ajuste = fitted(modele_poly))
plot(df$temps, df$construction, type = "l", col = "gray", main = "Ajustement polynomial (degré 2)", xlab = "Temps", ylab = "Production")
lines(df$temps, df$ajuste, col = "blue", lwd = 2)
legend("topleft", legend = c("Série", "Ajustement polynôme d'ordre 2"), col = c("gray", "blue"), lty = 1)
